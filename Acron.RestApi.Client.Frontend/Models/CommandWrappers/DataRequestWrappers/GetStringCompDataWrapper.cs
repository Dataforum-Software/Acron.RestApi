...
{
   internal class GetStringCompDataWrapper : ObservableObject, IWrapper
   {
      #region ctor
      public GetStringCompDataWrapper(RestClient? client = null)
      {
         _myDayDataRequest = new StringCompDataRequests(client);
         Name = string.Empty;
      ...
      #region Fields
      private StringCompDataRequests? _myDayDataRequest;
      #endregion
      ...
      public virtual string InputBodyText
      {
         ...
         set
         {
            try
            {
               var jsonstring = JsonConvert.DeserializeObject<GetStringCompDataRequestResource>(value);
      ...
      public GetStringCompDataRequestResource Input { get; set; }
      ...
      public async Task ExecuteMethod()
      {
         if (_myDayDataRequest == null)
            return;
         UpdateCustoms();
         (HasError, ErrorText, Response, Result) = await _myDayDataRequest.GetStringCompData(Input);
