...
{
   public interface IEventItem<U, V, W, X> where U : IEventSubItem<V, W, X> where V : IEventCell where W : IEventDataItem<X> where X : IEventDataCell
   {
      [SwaggerSchema("Unique name identifier of the event")]
      [SwaggerExampleValue("0001")]
      string ShortName { get; set; }

      [SwaggerSchema("Unique identifier of the event")]
      [SwaggerExampleValue(5020000001)]
      int EventId { get; set; }
      ...
      List<V> EventCells { get; set; }

      [SwaggerSchema("List of associated sub-items")]
      [SwaggerExampleValue(true)]
      List<U> SubItems { get; set; }
